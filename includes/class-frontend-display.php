...
class DPT_Frontend_Display {
            ...
            <ul class="dpt-dropdown-menu">
                <?php foreach ($languages as $code => $name): ?>
                    <?php if ($code !== $this->current_language): ?>
                        <li>
                            <a href="#" class="dpt-lang-link" data-lang="<?php echo esc_attr($code); ?>">
    ...
    private function render_inline_switcher($languages) {
        ...
        <div class="dpt-inline-flags">
            <?php foreach ($languages as $code => $name): ?>
                <a href="#" class="dpt-lang-link <?php echo $code === $this->current_language ? 'active' : ''; ?>" 
    ...
    private function render_popup_switcher($languages, $current_flag) {
        ...
        <div class="dpt-popup-overlay" style="display:none;">
            <div class="dpt-popup-content">
                <div class="dpt-popup-header">
                    <h3><?php _e('Seleziona Lingua', 'dynamic-translator'); ?></h3>
                    <button class="dpt-popup-close">&times;</button>
                </div>
                <div class="dpt-popup-body">
                    <div class="dpt-lang-grid">
                        <?php foreach ($languages as $code => $name): ?>
                            <a href="#" class="dpt-lang-card <?php echo $code === $this->current_language ? 'active' : ''; ?>" 
    ...
    public function ajax_change_language() {
        ...
        if (!in_array($new_language, $this->enabled_languages)) {
            wp_send_json_error(array('message' => __('Lingua non supportata', 'dynamic-translator')));
        }
        
        // Imposta cookie per lingua
        setcookie('dpt_current_lang', $new_language, time() + (30 * DAY_IN_SECONDS), '/');
        
        // Aggiorna lingua corrente
        $this->current_language = $new_language;
        
        wp_send_json_success(array(
            'message' => __('Lingua cambiata', 'dynamic-translator'),
    ...
    public function ajax_translate_element() {
        ...
        // Salva in cache
        $cache_handler->save_translation($cache_key, $translation, $content, $source_lang, $target_lang);
